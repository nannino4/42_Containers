...
**************************************************************************************************/
	...
	for (int i = 0; i < COUNT; ++i)
	{
		...
		ret.output << vec_int[i] << std::endl;
		ret.output << vec_str[i] << std::endl;
		ret.output << vec_str_range[i] << std::endl;
		ret.output << vec_str_copy[i] << std::endl;
	}

	vec_str.push_back(" a tutti");
	ret.output << (vec_str == vec_str_copy) << std::endl;			// false
	ret.output << (vec_str_range == vec_str_copy) << std::endl;		// true
	ret.output << (vec_str_range < vec_str_copy) << std::endl;		// false
...
// print vec_int //
	for (long unsigned int i = 0; i < vec_int.size(); ++i)
		ret.output << vec_int.at(i) << std::endl;
///////////////////
